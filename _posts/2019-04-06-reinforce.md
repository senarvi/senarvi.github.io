...
Instead, usually metrics such as BLEU and ROUGE are used, that compare n-gram statistics of the most likely sequence generated by the model to those of the reference sequence.
...
But during training the model is fed with the reference sequence (offset by one word), and used just to compute the next word probabilities.
